...
const moderationAction = await readFile(new URL('../lib/moderation-action.ts', import.meta.url), 'utf8');
...
test('server HTTP integrations use the Node 22 fetch primitive instead of legacy got', () => {
  ...
  for (const source of [hive, moderationAction, slack, report]) {
...
test('external HTTP writes reject non-success responses instead of silently accepting them', () => {
  ...
  assert.match(moderationAction, /response\.ok/);
